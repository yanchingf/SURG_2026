...
from data.star_io import parse_star_data
from data.star_io import save_processed_data
from data.star_io import get_coords_and_brightness
...
def star_to_graph_mapping(skycoords): # need to update per iteration
    ...
    build_graph(coords, brightness)


# astropy get constellation
def get_constellation_stars(star, table): # get all stars belonging to a constellation
    return
    ...
    for i in range(n-1):
        if graph.nodes[i].active == True:
            for j in range(i+1, n):
                if graph.nodes[j].active and g.adj[i][j] > 0:
                    plt.plot([ra[i].radian, ra[j].radian],[dec[i].radian, dec[j].radian],
                            color='gray', lw=0.8, alpha=0.5, zorder=1)
                    
    for i in range(n):

        color = (cm.tab10(graph.nodes[i].cluster_id) if graph.nodes[i].active else "gray")
